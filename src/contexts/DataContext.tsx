...
interface DataContextType {
  ...
  addLoanProduct: (product: Omit<LoanProduct, 'id' | 'createdDate' | 'lastUpdated'>) => void;
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  // Helper function to get organization-scoped localStorage key
  const getStorageKey = (baseKey: string): string => {
    try {
      const orgData = localStorage.getItem('current_organization');
      if (orgData) {
        const org = JSON.parse(orgData);
        // Use organization ID to scope the data
    ...
    // Fallback to default if no organization context
    return `default_${baseKey}`;
  };

  // Load data from localStorage on mount - OPTIMIZED
  useEffect(() => {
    const loadData = () => {
      try {
        // Data version for tracking schema changes
        const DATA_VERSION = '2.5';
        const currentVersion = localStorage.getItem('bvfunguo_data_version');
        const hasInitialized = localStorage.getItem('bvfunguo_initialized');
        
        // Update version if changed, but DON'T wipe data
        if (currentVersion !== DATA_VERSION) {
          localStorage.setItem('bvfunguo_data_version', DATA_VERSION);
        }
        
        // Only initialize on very first visit
        if (!hasInitialized) {
        ...
        // Load critical data first (for immediate UI rendering)
        setClients(safeParse('bvfunguo_clients'));
        setLoans(safeParse('bvfunguo_loans'));
        setLoanProducts(safeParse('bvfunguo_loan_products'));
        
        // Load remaining data
        setRepayments(safeParse('bvfunguo_repayments'));
        setSavingsAccounts(safeParse('bvfunguo_savings_accounts'));
        setSavingsTransactions(safeParse('bvfunguo_savings_transactions'));
        setShareholders(safeParse('bvfunguo_shareholders'));
        setShareholderTransactions(safeParse('bvfunguo_shareholder_transactions'));
        setExpenses(safeParse('bvfunguo_expenses'));
        setPayees(safeParse('bvfunguo_payees'));
        setBankAccounts(safeParse('bvfunguo_bank_accounts'));
        setTasks(safeParse('bvfunguo_tasks'));
        setKYCRecords(safeParse('bvfunguo_kyc_records'));
        setApprovals(safeParse('bvfunguo_approvals'));
        setFundingTransactions(safeParse('bvfunguo_funding_transactions'));
        setProcessingFeeRecords(safeParse('bvfunguo_processing_fee_records'));
        setDisbursements(safeParse('bvfunguo_disbursements'));
        setPayrollRuns(safeParse('bvfunguo_payroll_runs'));
        setJournalEntries(safeParse('bvfunguo_journal_entries'));
        setAuditLogs(safeParse('bvfunguo_audit_logs'));
        setTickets(safeParse('bvfunguo_tickets'));
        setGroups(safeParse('bvfunguo_groups'));

        const storedGuarantors = localStorage.getItem(getStorageKey('bvfunguo_guarantors'));
        const storedCollaterals = localStorage.getItem(getStorageKey('bvfunguo_collaterals'));
        const storedLoanDocuments = localStorage.getItem(getStorageKey('bvfunguo_loan_documents'));
        
        if (storedGuarantors) {
          setGuarantors(JSON.parse(storedGuarantors));
        }
        
        if (storedCollaterals) {
          setCollaterals(JSON.parse(storedCollaterals));
        }
        
        if (storedLoanDocuments) {
          setLoanDocuments(JSON.parse(storedLoanDocuments));
        }
      } catch (error) {
    ...
    // Initialize auto-backup system ONCE per session (not every page load)
  ...
  // Save data to localStorage with debouncing (OPTIMIZED for performance)
  // Single useEffect with debounce to prevent excessive localStorage writes
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      try {
        // Save all data in one go with requestIdleCallback for better performance
        const saveData = () => {
          localStorage.setItem(getStorageKey('bvfunguo_clients'), JSON.stringify(clients));
          localStorage.setItem(getStorageKey('bvfunguo_loans'), JSON.stringify(loans));
          localStorage.setItem(getStorageKey('bvfunguo_repayments'), JSON.stringify(repayments));
          localStorage.setItem(getStorageKey('bvfunguo_savings_accounts'), JSON.stringify(savingsAccounts));
          localStorage.setItem(getStorageKey('bvfunguo_savings_transactions'), JSON.stringify(savingsTransactions));
          localStorage.setItem(getStorageKey('bvfunguo_loan_products'), JSON.stringify(loanProducts));
          localStorage.setItem(getStorageKey('bvfunguo_shareholders'), JSON.stringify(shareholders));
          localStorage.setItem(getStorageKey('bvfunguo_shareholder_transactions'), JSON.stringify(shareholderTransactions));
          localStorage.setItem(getStorageKey('bvfunguo_expenses'), JSON.stringify(expenses));
          localStorage.setItem(getStorageKey('bvfunguo_payees'), JSON.stringify(payees));
          localStorage.setItem(getStorageKey('bvfunguo_bank_accounts'), JSON.stringify(bankAccounts));
          localStorage.setItem(getStorageKey('bvfunguo_tasks'), JSON.stringify(tasks));
          localStorage.setItem(getStorageKey('bvfunguo_kyc_records'), JSON.stringify(kycRecords));
          localStorage.setItem(getStorageKey('bvfunguo_audit_logs'), JSON.stringify(auditLogs));
          localStorage.setItem(getStorageKey('bvfunguo_tickets'), JSON.stringify(tickets));
          localStorage.setItem(getStorageKey('bvfunguo_groups'), JSON.stringify(groups));
          localStorage.setItem(getStorageKey('bvfunguo_approvals'), JSON.stringify(approvals));
          localStorage.setItem(getStorageKey('bvfunguo_disbursements'), JSON.stringify(disbursements));
          localStorage.setItem(getStorageKey('bvfunguo_processing_fee_records'), JSON.stringify(processingFeeRecords));
          localStorage.setItem(getStorageKey('bvfunguo_funding_transactions'), JSON.stringify(fundingTransactions));
          localStorage.setItem(getStorageKey('bvfunguo_payroll_runs'), JSON.stringify(payrollRuns));
          localStorage.setItem(getStorageKey('bvfunguo_journal_entries'), JSON.stringify(journalEntries));
          localStorage.setItem(getStorageKey('bvfunguo_guarantors'), JSON.stringify(guarantors));
          localStorage.setItem(getStorageKey('bvfunguo_collaterals'), JSON.stringify(collaterals));
          localStorage.setItem(getStorageKey('bvfunguo_loan_documents'), JSON.stringify(loanDocuments));
        };
        
        // Use requestIdleCallback if available, otherwise just save
    ...
    clients, loans, repayments, savingsAccounts, savingsTransactions,
    loanProducts, shareholders, shareholderTransactions, expenses, payees,
    bankAccounts, tasks, kycRecords, auditLogs, tickets, groups,
    approvals, disbursements, processingFeeRecords, fundingTransactions,
    payrollRuns, journalEntries, guarantors, collaterals, loanDocuments
  ]);
  ...
  const clearAllData = () => {
    ...
    localStorage.removeItem('bvfunguo_clients');
    localStorage.removeItem('bvfunguo_loans');
    localStorage.removeItem('bvfunguo_loan_products');
    localStorage.removeItem('bvfunguo_savings_accounts');
    localStorage.removeItem('bvfunguo_savings_transactions');
    localStorage.removeItem('bvfunguo_repayments');
    localStorage.removeItem('bvfunguo_shareholders');
    localStorage.removeItem('bvfunguo_shareholder_transactions');
    localStorage.removeItem('bvfunguo_expenses');
    localStorage.removeItem('bvfunguo_payees');
    localStorage.removeItem('bvfunguo_bank_accounts');
    localStorage.removeItem('bvfunguo_tasks');
    localStorage.removeItem('bvfunguo_kyc_records');
    localStorage.removeItem('bvfunguo_audit_logs');
    localStorage.removeItem('bvfunguo_tickets');
    localStorage.removeItem('bvfunguo_groups');
    localStorage.removeItem('bvfunguo_approvals');
    localStorage.removeItem('bvfunguo_disbursements');
    localStorage.removeItem('bvfunguo_processing_fee_records');
    localStorage.removeItem('bvfunguo_funding_transactions');
    localStorage.removeItem('bvfunguo_payroll_runs');
  ...
  const clearShareholdersAndBanks = () => {
    ...
    // Clear from localStorage using organization-scoped keys
    ...
    console.log('✅ Shareholders and bank accounts cleared');
  };

  // ============= CLIENT FUNCTIONS =============

  const addClient = (clientData: Omit<Client, 'id' | 'joinDate' | 'lastUpdated'>) => {
    // Generate 4-character ID: C + 3-digit number
    const maxId = clients.reduce((max, client) => {
      const match = client.id.match(/^C(\d{3})$/);
    ...
    syncToSupabase('create', 'client', newClient);
  };
  ...
  const updateClient = (id: string, updates: Partial<Client>) => {
    ...
    // Sync to Supabase in background
    syncToSupabase('update', 'client', updates, id);
  };
  ...
  const deleteClient = (id: string) => {
    ...
    // Sync to Supabase in background
    syncToSupabase('delete', 'client', null, id);
  };
  ...
  const addLoan = async (loanData: Omit<Loan, 'id' | 'applicationDate'>) => {
    ...
    if (product && product.processingFee > 0) {
      ...
      // IMPORTANT: Sync loan FIRST, then processing fee (to satisfy foreign key constraints)
      // Sync loan to Supabase FIRST (AWAIT to ensure it completes)
      await syncToSupabase('create', 'loan', newLoan);
      
      // Then sync processing fee to Supabase (after loan exists)
      await syncToSupabase('create', 'processing_fee_record', newProcessingFeeRecord);
    } else {
      // No processing fee - just sync the loan
      await syncToSupabase('create', 'loan', newLoan);
    }
    
    // Sync approval to Supabase LAST (after loan exists)
    await syncToSupabase('create', 'approval', newApproval);
  ...
  const updateLoan = (id: string, updates: Partial<Loan>) => {
    ...
    // Sync to Supabase
    syncToSupabase('update', 'loan', updates, id);
  };
  ...
  const addRepayment = (repaymentData: Omit<Repayment, 'id' | 'createdDate'>) => {
    ...
    // Sync to Supabase
    syncToSupabase('create', 'repayment', newRepayment);
  };
  ...
  const addLoanProduct = (productData: Omit<LoanProduct, 'id' | 'createdDate' | 'lastUpdated'>) => {
    ...
    // Sync to Supabase
    syncToSupabase('create', 'loan_product', newProduct);
  };
  ...
  const updateLoanProduct = (id: string, updates: Partial<LoanProduct>) => {
    ...
    // Sync to Supabase
    syncToSupabase('update', 'loan_product', updates, id);
  };
  ...
  const deleteLoanProduct = (id: string) => {
    ...
    // Sync to Supabase
    syncToSupabase('delete', 'loan_product', {}, id);
  };
  ...
  const updateExpense = (id: string, updates: Partial<Expense>) => {
    ...
    // Sync to Supabase
    syncToSupabase('update', 'expense', updates, id);
  };
  ...
  const deleteExpense = (id: string) => {
    ...
    // Sync to Supabase
    syncToSupabase('delete', 'expense', null, id);
  };
  ...
  const updateBankAccount = (id: string, updates: Partial<BankAccount>) => {
    setBankAccounts(bankAccounts.map(acc => 
      acc.id === id ? { ...acc, ...updates, lastUpdated: new Date().toISOString().split('T')[0] } : acc
    ));
  };
    ...
    loans.forEach(loan => {
      ...
      if (!hasApproval) {
        ...
        const newApproval: Approval = {
          id: approvalId,
          type: 'loan_application',
          title: `Loan Application - ${loan.clientName}`,
          description: `${loan.productName} loan application for KES ${loan.principalAmount.toLocaleString()} over ${loan.term} ${loan.termUnit.toLowerCase()}`,
