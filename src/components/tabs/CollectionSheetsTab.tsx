...
export function CollectionSheetsTab() {
  ...
  const { loans, clients, repayments, bankAccounts } = useData();
  ...
  const repaymentsForDate = repayments.filter(r => {
    ...
    // ✅ Match both camelCase and snake_case, check for 'Approved' or 'completed'
    return paymentDate === selectedDate && (status === 'Approved' || status === 'completed');
  });

  // 🔍 DEBUG: Log repayments data
  console.log('📊 COLLECTIONS DEBUG:');
  console.log('  All repayments:', repayments.length);
  console.log('  Selected date:', selectedDate);
  console.log('  Repayments for date:', repaymentsForDate.length);
  if (repayments.length > 0) {
    console.log('  Sample repayment:', repayments[0]);
  }
  if (repaymentsForDate.length > 0) {
    console.log('  Sample matching repayment:', repaymentsForDate[0]);
  }
  ...
  const todaysCollections = [
    ...
    ...repaymentsForDate.map(repayment => {
      // ✅ Join with loans to get user-friendly loan ID
      const loan = loans.find(l => l.id === repayment.loanId || l.id === repayment.loan_id);
      
      // ✅ Join with clients to get name and phone
      const client = clients.find(c => c.id === (repayment.clientId || repayment.client_id));
      
      // ✅ Join with bank accounts to get "Paid To" column
      const bankAccountId = repayment.bankAccountId || repayment.bank_account_id;
      const bankAccount = bankAccountId ? bankAccounts?.find((ba: any) => ba.id === bankAccountId) : null;
      const paidTo = bankAccount 
        ? `${bankAccount.name || bankAccount.bankName || bankAccount.bank_name || 'Unknown Bank'}` 
        : bankAccountId ? 'Bank Account (Unknown)' : '-';
      
      console.log('🔍 Mapping repayment to collection:', {
        repaymentId: repayment.id,
        loanUUID: repayment.loanId || repayment.loan_id,
        foundLoan: loan ? { id: loan.id, loanId: loan.loanId, clientId: loan.clientId } : 'NOT FOUND',
        clientUUID: repayment.clientId || repayment.client_id,
        foundClient: client ? { id: client.id, name: client.name, phone: client.phone } : 'NOT FOUND',
        bankAccountId: repayment.bankAccountId || repayment.bank_account_id,
        foundBankAccount: bankAccount ? { id: bankAccount.id, name: bankAccount.name } : 'NOT FOUND',
        paymentMethod: repayment.paymentMethod || repayment.payment_method
      });
      
      return {
        uniqueKey: `paid-${repayment.id}`,
        loanId: loan?.loanId || loan?.loan_id || 'Unknown', // ✅ User-friendly loan ID (LN00001)
        loanUUID: loan?.id || repayment.loanId, // Keep UUID for lookups
        clientId: client?.id || repayment.clientId || '',
        clientName: client?.name || '',
        phoneNumber: client?.phone || '',
        loanAmount: loan?.principalAmount || loan?.loanAmount || 0,
        dueAmount: repayment.amount || 0,
        dueDate: selectedDate,
        status: 'Paid',
        paidAmount: repayment.amount || 0,
        paymentMethod: repayment.paymentMethod || repayment.payment_method || '-',
        collectedBy: paidTo // Bank account details
      };
    }),
    ...
    ...activeLoans
      ...
      .map(loan => {
        ...
        return {
          uniqueKey: `pending-${loan.id}`, // ✅ Unique key for React
  ...
  console.log('📊 TODAY\'S COLLECTIONS:', todaysCollections);
              ...
              <table className="w-full">
                <thead className={`sticky top-0 ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  <tr>
                    ...
                    <th className={`px-6 py-3 text-left text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Paid To</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredCollections.map((collection) => (
                    <tr key={collection.uniqueKey} className={`border-t ${isDark ? 'border-gray-700 hover:bg-gray-700' : 'border-gray-100 hover:bg-gray-50'}`}>
