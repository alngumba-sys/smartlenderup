...
import { ClientLoanNotificationCard } from '../ClientLoanNotificationCard';

export function NotificationsTab() {
  const { isDark } = useTheme();
  const { notifications, markAllNotificationsAsRead, getUnreadNotificationsCount, markNotificationAsRead } = useData();
    ...
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="client_application">Client Applications</option>
      ...
      <div className="space-y-3">
        {filteredNotifications.map((notification) => {
          ...
          if (notification.category === 'client_application' && notification.actionRequired) {
            return (
              <ClientLoanNotificationCard
                key={notification.id}
                notification={notification}
                onActionTaken={() => setRefreshKey(prev => prev + 1)}
              />
            );
          }
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-500">
                        ...
                        {notification.relatedId && (
                          <span className="px-2 py-0.5 bg-gray-100 rounded">
                            #{notification.relatedId.slice(0, 8)}
                          </span>
                        )}
                      </div>
