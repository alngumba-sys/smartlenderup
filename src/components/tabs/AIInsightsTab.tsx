...
export function AIInsightsTab() {
  ...
  // Calculate actual high-risk clients from loan data
  ...
  const actualHighRiskClients = loansWithArrears.map(loan => {
    const client = clients.find(c => c.clientId === loan.clientId);
    return {
      id: loan.clientId,
      name: client?.fullName || loan.clientName,
      creditScore: 100 - (loan.daysInArrears * 2),
      status: loan.status,
      predictedDefaultDate: loan.daysInArrears > 30 ? '2025-02-28' : loan.daysInArrears > 7 ? '2025-03-15' : '2025-04-30',
      daysInArrears: loan.daysInArrears,
      arrearsAmount: loan.outstandingBalance
    };
  });

  const highRiskClients = actualHighRiskClients.length > 0 ? actualHighRiskClients : [];
  ...
  const pricingSimulation = loanProducts.map(product => {
  ...
  const churnRiskClients = highRiskClients.map(client => {
    const clientData = clients.find(c => c.clientId === client.id);
    return {
      id: client.id,
      name: client.name,
      phone: clientData?.phoneNumber || '0700000000',
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-amber-700 text-sm mt-1">
                  Client CL-019 (Halima Abdi): Multiple small deposits followed by withdrawal request. 
