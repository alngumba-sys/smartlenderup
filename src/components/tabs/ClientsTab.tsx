...
export function ClientsTab({ onClientSelect }: ClientsTabProps) {
  ...
  // Recalculate all client credit scores on component mount and when loans/repayments change
  useEffect(() => {
    ...
    clients.forEach(client => {
      ...
      if (newScore !== client.creditScore || newRiskRating !== client.riskRating) {
        updateClient(client.id, { creditScore: newScore, riskRating: newRiskRating });
        hasUpdates = true;
      }
    });
    if (hasUpdates) {
  ...
  const activeClients = clientsOfType.filter(c => c.status === 'Active').length;
  const clientsInArrears = clientsOfType.filter(c => c.status === 'Inactive').length;
  ...
  const totalOutstandingAll = clientsOfType.reduce((sum, client) => {
    ...
    return sum + activeLoans.reduce((loanSum, l) => loanSum + l.outstandingBalance, 0);
  }, 0);
  ...
  return (
    <div className="p-6 space-y-6 bg-transparent">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        >
          <div className="flex items-start gap-3">
            {clientTypeTab === 'individual' ? (
              <User className="size-6 text-blue-600 dark:text-blue-400 flex-shrink-0 mt-1" />
            ) : (
              <Building2 className="size-6 text-blue-600 dark:text-blue-400 flex-shrink-0 mt-1" />
            )}
            <div className="flex-1">
              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                {clientTypeTab === 'individual' ? 'Total Individuals' : 'Total Businesses'}
              </p>
              <p className={`text-2xl mt-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>{totalClients}</p>
            </div>
          </div>
        </div>
      ...
      {/* Search and Filters */}
      <div className="dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 px-[16px] py-[7px] bg-[rgb(17,17,32)]">
            ...
            <table className="w-full">
              <thead className={`sticky top-0 ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                <tr>
                  ...
                  <th className={`px-4 py-2 text-right text-[11px] ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Outstanding</th>
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        ...
                        <p className="text-gray-900 dark:text-white">
                          {clients.filter(c => c.status === 'Active').reduce((sum, client) => {
                            const clientLoans = loans.filter(l => l.clientId === client.id && (l.status === 'Active' || l.status === 'Disbursed'));
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {useData.clients.filter(c => c.status === 'Good Standing')
                        ...
                        .slice(0, 10)
                        .map(client => {
                          const clientLoans = useData.loans.filter(l => l.clientId === client.id && (l.status === 'Active' || l.status === 'In Arrears'));
                      ...
                      <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        ...
                        <p className="text-gray-900 dark:text-white">
                          KES {(useData.clients.filter(c => c.status === 'In Arrears').reduce((sum, client) => {
                            const clientLoans = useData.loans.filter(l => l.clientId === client.id && l.daysInArrears > 0);
                            return sum + clientLoans.reduce((loanSum, l) => loanSum + l.outstandingBalance, 0);
                          }, 0) / 1000).toFixed(0)}K
                        </p>
                      </div>
