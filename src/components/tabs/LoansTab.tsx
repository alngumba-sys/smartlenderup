  ...
  const confirmDeleteLoan = () => {
    ...
    try {
      ...
      toast.success('Loan deleted successfully', {
        description: `Loan ${loanToDelete.id} has been removed from the system`,
      });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {activeSubTab !== 'guarantors' && activeSubTab !== 'comments' && activeSubTab !== 'repayment-schedule' && (() => {
        // Only count active/disbursed loans for metrics
        const activeDisbursedLoans = filteredLoans.filter(l => l.status === 'Active' || l.status === 'Disbursed');
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl mt-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  KES {(activeDisbursedLoans.reduce((sum, l) => sum + (l.principalAmount || 0), 0) / 1000000).toFixed(2)}M
                </p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl mt-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  KES {((activeDisbursedLoans.reduce((sum, l) => sum + (l.outstandingBalance || 0), 0)) / 1000000).toFixed(2)}M
                </p>
              </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{loan.id}</p>
                    ...
                    {filteredLoans.map(loan => {
                      ...
                      return (
                        <tr key={loan.id} className={`border-t ${isDark ? 'border-gray-700 hover:bg-gray-700' : 'border-gray-100 hover:bg-gray-50'}`}>
                          <td className={`px-4 py-2 text-xs ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{loan.id}</td>
                          <td className={`px-4 py-2 text-center text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                            {loan.applicationDate || loan.createdDate || '-'}
                          </td>
                          <td className={`px-4 py-2 text-xs ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{client?.name}</td>
                          <td className={`px-4 py-2 text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{client?.id}</td>
      ...
      {detailModalLoan && (
        <LoanDetailsModal
          ...
          onClose={() => setDetailModalLoan(null)}
        />
      )}
                    ...
                    {loans.map(loan => {
                      ...
                      return (
                        <option key={loan.id} value={loan.id}>
                          {loan.id} - {client?.name || 'Unknown Client'}
                        </option>
                      );
                    })}
                  </select>
