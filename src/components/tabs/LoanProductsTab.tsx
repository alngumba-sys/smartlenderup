...
export function LoanProductsTab() {
  ...
  const getProductMetrics = (productId: string) => {
    ...
    const parRate = activeLoans.length > 0 ? (parLoans / activeLoans.length) * 100 : 0;
    ...
    <div className="p-6 space-y-6 dark:bg-gray-900 bg-[rgb(17,17,32)]">
      ...
      {showNewProductModal && (
        <ProductFormModal
          ...
          onSubmit={async (productData) => {
            ...
            const isConnected = await ensureSupabaseConnection('create loan product');
            if (!isConnected) {
      ...
      {deletingProduct && (
        <DeleteProductModal
          ...
          onDelete={async () => {
            try {
              await deleteLoanProduct(deletingProduct);
              setDeletingProduct(null);
            } catch (error) {
              // Error is already handled in deleteLoanProduct with toast
