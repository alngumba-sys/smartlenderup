...
export function AddLoanProductModal({ isDark, onClose, onAdd }: AddLoanProductModalProps) {
  const { addLoanProduct } = useData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newProduct = await addLoanProduct(formData);
      onAdd(newProduct);
      onClose();
    } catch (err: any) {
      setError(err.message || 'Failed to create loan product');
      setLoading(false);
    }
  };
