...
export function RecordPaymentModal({ isOpen, onClose, onSubmit, preselectedLoanId }: RecordPaymentModalProps) {
  ...
  // Filter destination accounts (where payment is received)
            ...
            <div className="bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg p-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white">{selectedLoan?.id}</p>
                </div>
                <div>
                  <span className="text-xs text-gray-500 dark:text-gray-400">Outstanding Balance:</span>
                  <p className="text-emerald-600 dark:text-emerald-400">{currencyCode} {selectedLoan?.outstandingBalance.toLocaleString()}</p>
                </div>
                <div>
                  ...
                  .map(loan => {
                    ...
                    return (
                      <option key={loan.id} value={loan.id}>
                        {loan.id} - {client?.name} ({loan.status}) - Outstanding: {currencyCode} {loan.outstandingBalance.toLocaleString()}
                      </option>
                    );
                  })}
              </select>
              ...
              <div className="col-span-3 p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg">
                <div className="grid grid-cols-4 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-emerald-700">{currencyCode} {selectedLoan.outstandingBalance.toLocaleString()}</p>
                  </div>
                  <div>
            ...
            <div className="col-span-3">
              ...
              {destinationBankAccounts.length === 0 && (
                <p className="text-amber-600 dark:text-amber-400 text-xs mt-1">
                  ⚠️ No active bank or mobile money accounts found. Please add an account in Settings.
                </p>
              )}
            </div>
