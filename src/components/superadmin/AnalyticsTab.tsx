  ...
  const refreshStats = async () => {
    ...
    try {
      const [orgsResult, clientsResult, loansResult, repaymentsResult] = await Promise.all([
        ...
        supabase.from('clients').select('id, client_type, organization_id, created_at'),
        supabase.from('loans').select('id, organization_id, status, principal_amount, total_repayable, total_paid, disbursement_date, created_at'),
        supabase.from('repayments').select('id, amount, status, payment_date')
      ]);
      
      if (orgsResult.error || clientsResult.error || loansResult.error || repaymentsResult.error) {
        console.error('❌ Supabase error:', {
          orgs: orgsResult.error,
          clients: clientsResult.error,
          loans: loansResult.error,
          repayments: repaymentsResult.error
        });
