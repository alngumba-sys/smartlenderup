...
function ChartContainer({ children, config, className, isMounted = true }: { children: React.ReactNode; config: ChartConfig; className?: string; isMounted?: boolean }) {
  return (
    <div className={className} style={{ width: '100%', minWidth: '100px', position: 'relative' }}>
...
export function CollectionsReport({ dateRange }: ReportProps) {
  ...
  const officerCollections = activeLoans.reduce((acc, loan) => {
    ...
    const expected = loan.totalRepayable || (loan.principalAmount + (loan.totalInterest || 0));
    const collected = loan.paidAmount || 0;
    ...
    <div className="p-6 bg-gray-100 dark:bg-gray-900 min-h-screen print:p-0 print:bg-white print:min-h-0">
      ...
      <div className="w-[210mm] h-[297mm] box-border overflow-hidden mx-auto bg-white dark:!bg-white shadow-lg mb-6 p-[8mm] flex flex-col print:shadow-none print:mb-0 print:mx-0">
        ...
        <div className="mb-4">
          ...
          <ChartContainer config={chartConfig} className="h-[200px] w-full" isMounted={isMounted}>
            <RechartsLineChart
              accessibilityLayer
              data={monthlyData}
              margin={{
