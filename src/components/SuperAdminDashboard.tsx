...
import { 
  ...
  Filter, CheckCircle, XCircle, Clock, Eye, Ban, CheckSquare, RefreshCw, Database, Key, MessageSquare
} from 'lucide-react';
...
export function SuperAdminDashboard({ onClose }: SuperAdminDashboardProps) {
  ...
  useEffect(() => {
    const loadUnreadCount = async () => {
      try {
        ...
        if (!error && data !== null) {
          setUnreadMessagesCount(data as unknown as number);
        }
      } catch (err) {
  ...
  // AUTO-SYNC: Sync localStorage to Supabase when Super Admin opens
  useEffect(() => {
    const autoSync = async () => {
      console.log('🔄 Super Admin opened - auto-syncing data to Supabase...');
      setIsSyncing(true);
      
      try {
        const report = await syncAllDataToSupabase();
        
        const totalSynced = report.organizations.synced + report.clients.synced + 
                           report.loans.synced + report.repayments.synced;
        
        if (totalSynced > 0) {
          console.log(`✅ Auto-synced ${totalSynced} records to Supabase`);
          toast.success(`Synced ${totalSynced} records to database`, { duration: 3000 });
        } else {
          console.log('✅ All data already in sync');
        }
      } catch (error) {
...
function LenderManagementTab() {
  ...
  const [isLoading, setIsLoading] = useState(true);
  const [resettingPasswordOrg, setResettingPasswordOrg] = useState<any>(null);
  ...
  const resetPassword = async () => {
    if (!resettingPasswordOrg) return;
    ...
    try {
      console.log(`🔑 Resetting password for organization ${resettingPasswordOrg.id}...`);
      ...
      const { error } = await supabase
        ...
        .eq('id', resettingPasswordOrg.id);
      
      if (error) {
        console.error('❌ Supabase password reset error:', error);
        toast.error('Failed to reset password in database');
        return;
      }
      
      console.log('✅ Password reset in Supabase successfully');
      
      // Update in localStorage
      db.updateOrganization(resettingPasswordOrg.id, { password: newPassword });
      console.log('💾 Updated localStorage cache');
      
      toast.success(`Password reset successfully for ${resettingPasswordOrg.organization_name}`);
      setResettingPasswordOrg(null);
      setNewPassword('');
      setConfirmPassword('');
    } catch (error) {
  ...
  return (
    <div>
      ...
      {resettingPasswordOrg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0, 0, 0, 0.85)' }}>
          <div className="max-w-md w-full rounded-xl shadow-2xl" style={{ backgroundColor: '#1e4a5f', border: '1px solid rgba(245, 158, 11, 0.3)' }}>
            {/* Header */}
            <div className="px-6 py-4 flex items-center gap-3" style={{ backgroundColor: '#164459', borderBottom: '1px solid rgba(255, 255, 255, 0.1)' }}>
              <div className="p-2 rounded-lg" style={{ backgroundColor: 'rgba(245, 158, 11, 0.2)' }}>
                <Key className="size-5" style={{ color: '#f59e0b' }} />
              </div>
              <div>
                <h3 className="text-lg font-semibold" style={{ color: '#e8d1c9' }}>Reset Password</h3>
                <p className="text-xs" style={{ color: 'rgba(232, 209, 201, 0.6)' }}>Set a new password for this organization</p>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                <p className="text-sm mb-2" style={{ color: '#e8d1c9' }}>
                  Resetting password for <span className="font-semibold" style={{ color: '#f59e0b' }}>{resettingPasswordOrg.organization_name}</span>
                </p>
                <p className="text-xs" style={{ color: 'rgba(232, 209, 201, 0.6)' }}>
                  Username: <span className="font-mono" style={{ color: '#3b82f6' }}>{resettingPasswordOrg.username}</span>
                </p>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 rounded-lg font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  style={{ 
                    backgroundColor: (newPassword && confirmPassword && newPassword === confirmPassword && newPassword.length >= 8) ? '#f59e0b' : '#92400e', 
                    color: '#ffffff' 
                  }}
                >
                  Reset Password
...
function BorrowerManagementTab() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      await Promise.all([refreshClients(), refreshOrganizations(), refreshLoans()]);
      setIsLoading(false);
    };
    loadData();
  }, []);
  ...
  return (
    <div>
      ...
      {/* Clients Table */}
      <div className="rounded-lg overflow-hidden" style={{ border: '1px solid rgba(232, 209, 201, 0.1)', maxHeight: '600px', overflowY: 'auto' }}>
        <table className="w-full">
          <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
            <tr style={{ backgroundColor: '#154F73' }}>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium" style={{ color: '#e8d1c9' }}>Date Loan Approved</th>
          ...
          <tbody>
            {paginatedClients.length === 0 ? (
              ...
              paginatedClients.map((client, index) => {
                // Get latest approved loan date for this client
                const clientLoans = loans.filter(l => l.client_id === client.id && l.status === 'approved');
                const latestApprovedLoan = clientLoans.sort((a, b) => 
                  new Date(b.created_at || b.updated_at || 0).getTime() - new Date(a.created_at || a.updated_at || 0).getTime()
                )[0];
                const approvalDate = latestApprovedLoan?.approved_date || latestApprovedLoan?.created_at || latestApprovedLoan?.updated_at;
                const formattedDate = approvalDate ? new Date(approvalDate).toLocaleDateString('en-GB', { 
                  day: '2-digit', 
                  month: 'short', 
                  year: 'numeric' 
                }) : 'N/A';
