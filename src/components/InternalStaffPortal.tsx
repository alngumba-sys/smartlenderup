...
export function InternalStaffPortal({ onClientSelect, triggerTab }: InternalStaffPortalProps) {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'approvals', label: 'Approvals', icon: GitBranch, permission: 'approveLoans' },
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'clients':
        return <ClientsTab onClientSelect={onClientSelect} />;
      case 'loans':
        return <LoansTab />;
      case 'loan-calculator':
        return <LoanCalculatorTab />;
      case 'approvals':
        return <ApprovalsTab />;
      case 'loan-reconciliation':
        return <LoanReconciliationTab />;
      case 'payments':
        return <PaymentsTab />;
      case 'collection-sheets':
        return <CollectionSheetsTab />;
      case 'credit-scoring':
  ...
  return (
    <div className={`min-h-screen flex flex-col overflow-hidden transition-colors ${isDark ? 'dark' : ''}`} style={{ 
      ...
      {/* Main Content Area */}
      <div className="flex-1 overflow-auto p-6" style={{ backgroundColor: 'transparent' }}>
