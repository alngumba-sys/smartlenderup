...
export function TrialBanner({ organizationId, onPayNowClick }: TrialBannerProps) {
  ...
  return (
    <>
      <div 
        className="relative px-3 py-1.5 mb-3 rounded-md shadow-md animate-in fade-in slide-in-from-top-2 duration-500"
        style={{ 
          background: style.bg,
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 flex-1">
            ...
            <div className="flex items-center gap-2 flex-1">
              <h3 className="text-xs text-white">{message.title}</h3>
              <div className="px-1.5 py-0.5 rounded-full text-[10px]" style={{ 
                backgroundColor: 'rgba(255, 255, 255, 0.2)',
                color: '#fff'
              }}>
                {daysRemaining <= 0 ? 'EXPIRED' : `${daysRemaining}d left`}
              </div>
              <p className="text-[10px] ml-1" style={{ color: 'rgba(255, 255, 255, 0.85)' }}>
                {message.description}
              </p>
            </div>
          </div>
