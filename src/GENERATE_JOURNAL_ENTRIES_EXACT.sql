-- ==========================================
-- GENERATE JOURNAL ENTRIES - EXACT SCHEMA VERSION
-- SmartLenderUp - Uses your exact column names
-- ==========================================
...
BEGIN
  FOR loan_rec IN 
    SELECT 
      ...
      COALESCE(l.disbursement_date, l.approved_date, l.created_at)::DATE as disburse_date,
      COALESCE(c.name, c.first_name || ' ' || COALESCE(c.last_name, ''), 'Client') as client_display
    FROM loans l
    LEFT JOIN clients c ON l.client_id = c.id
    WHERE l.organization_id = org_id
    ORDER BY COALESCE(l.disbursement_date, l.approved_date, l.created_at), l.loan_number
  LOOP
...
SELECT '═══════════════════════════════════════════════════════════' as divider;
SELECT '✅ DOUBLE-ENTRY VERIFICATION (Should be empty if balanced)' as report_title;
SELECT '═══════════════════════════════════════════════════════════' as divider;
...
-- ✅ 46 lines for loan disbursements (2 per loan)
-- ✅ 69 lines for payment receipts (3 per payment)
-- 
-- Chart of Accounts Balances:
-- - Loans Receivable: Outstanding loan balances
-- - Interest Income: KES 115,000 (23 × 5,000)
-- - M-Pesa Account: Collections via M-Pesa
-- - Cash in Hand: Collections via Cash
-- - Cash at Bank: Bank transfers minus disbursements
-- - Share Capital: Total capital deployed for loans
-- ==========================================
