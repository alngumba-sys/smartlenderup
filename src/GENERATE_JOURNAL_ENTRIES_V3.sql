-- ==========================================
-- GENERATE JOURNAL ENTRIES - VERSION 3
...
-- STEP 1: Drop and recreate tables to ensure correct schema
-- ==========================================

DROP TABLE IF EXISTS journal_entry_lines CASCADE;
DROP TABLE IF EXISTS journal_entries CASCADE;

-- STEP 2: Create journal_entries table with correct structure
-- ==========================================
...
-- STEP 3: Create journal_entry_lines table
-- ==========================================
...
-- STEP 5: Insert Standard Chart of Accounts
-- ==========================================
...
-- ==========================================
-- STEP 6: Show what we're processing
-- ==========================================
...
-- ==========================================
-- STEP 7: Generate Journal Entries for LOAN DISBURSEMENTS
-- ==========================================
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO journal_entries (
      id, organization_id, entry_number, entry_date, description, reference,
...
-- ==========================================
-- STEP 8: Generate Journal Entries for PAYMENTS
-- ==========================================
...
-- ==========================================
-- STEP 9: Update Chart of Accounts Balances
-- ==========================================

-- Loans Receivable: Calculate as (Total Disbursed - Total Principal Paid)
UPDATE chart_of_accounts
SET balance = (
  (SELECT COALESCE(SUM(amount), 0) FROM loans WHERE organization_id = '958b04d3-ccc0-4d9d-8af7-01ba5cda3bb9'::UUID)
  -
  (SELECT COALESCE(SUM(principal_amount), 0) FROM repayments WHERE organization_id = '958b04d3-ccc0-4d9d-8af7-01ba5cda3bb9'::UUID)
), updated_at = NOW()
...
-- Cash at Bank (bank transfers received minus total disbursed)
UPDATE chart_of_accounts
SET balance = (
  (SELECT COALESCE(SUM(amount), 0)
   FROM repayments
...
SELECT '═══════════════════════════════════════════════════════════' as divider;
SELECT '✅ DOUBLE-ENTRY VERIFICATION' as report_title;
SELECT '═══════════════════════════════════════════════════════════' as divider;

SELECT 
  'Total Debits from Lines: ' || 'KES ' || TO_CHAR(SUM(debit), 'FM999,999,999.00') as check1,
  'Total Credits from Lines: ' || 'KES ' || TO_CHAR(SUM(credit), 'FM999,999,999.00') as check2,
  'Difference (should be 0): ' || 'KES ' || TO_CHAR(SUM(debit) - SUM(credit), 'FM999,999,999.00') as verification
FROM journal_entry_lines
WHERE organization_id = '958b04d3-ccc0-4d9d-8af7-01ba5cda3bb9';
