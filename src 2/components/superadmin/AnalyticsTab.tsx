...
export function AnalyticsTab() {
  ...
  const refreshStats = async () => {
    ...
    try {
      const [orgsResult, clientsResult, loansResult, paymentsResult] = await Promise.all([
        supabase.from('organizations').select('id, name, country, status, created_at'),
        supabase.from('clients').select('id, business_type, organization_id, created_at'),
        supabase.from('loans').select('id, organization_id, status, principal_amount, total_amount, paid_amount, disbursed_at, created_at'),
  ...
  const individualBorrowerCount = filteredClients.filter(c => c.business_type === 'individual').length;
  const groupBorrowerCount = filteredClients.filter(c => c.business_type === 'group').length;
  ...
  const getMonthlyDisbursements = () => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthDisbursements = filteredLoans.filter(loan => {
        if (!loan.disbursed_at && !loan.created_at) return false;
        
        const loanDate = new Date(loan.disbursed_at || loan.created_at);
