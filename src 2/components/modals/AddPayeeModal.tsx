...
export function AddPayeeModal({ onClose }: AddPayeeModalProps) {
  const { isDark } = useTheme();
  const { addPayee } = useData();
  const [formData, setFormData] = useState({
    name: '',
    type: 'Vendor' as 'Vendor' | 'Supplier' | 'Service Provider' | 'Employee' | 'Contractor' | 'Other',
    category: 'Other' as 'Employee' | 'Utilities' | 'Rent' | 'Services' | 'Suppliers' | 'Other',
    phone: '',
    email: '',
    kraPin: '',
    bankName: '',
