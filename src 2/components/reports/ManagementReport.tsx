...
function LineChart({ xAxis, series, height, margin }: any) {
  return (
...
function BarChart({ xAxis, series, height, margin }: any) {
  return (
    <ResponsiveContainer width="100%" height={height} minWidth={0}>
...
function PieChart({ series, height, margin }: any) {
...
export function ManagementReport({ dateRange }: ReportProps) {
  ...
  const totalRepaidAmount = loans.reduce((sum, l) => sum + (l.principalAmount - l.outstandingBalance), 0);
  const overallRecoveryRate = safePercentage(totalRepaidAmount, totalDisbursedAmount, 2);
  
  const openLoansRepaidAmount = activeLoans.reduce((sum, l) => sum + (l.principalAmount - l.outstandingBalance), 0);
